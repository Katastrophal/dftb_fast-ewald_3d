...
module dftbp_dftb_coulomb
   ...
   type :: TCoulomb
      ...
      logical :: stockMatrixReady_ = .false.
      ...
      logical :: cellInitialised_ = .false.
   ...
   contains
      ...
      !> Whether the original interaction matrix has been built
      procedure :: hasAtomicMatrix
...
contains
   ...
   !> ----------------------------------------------------------------------------
   !> FFT-Ewald engine: runtime switch + helpers
   !> ----------------------------------------------------------------------------
   !> Opt in to the fast Ewald engine. The original DFTB+ matrix path remains
   !> the default so methods that need its atomic gamma matrix continue to work.
   function useFftEwald() result(useFft)
      logical :: useFft
      character(len=16) :: val
      integer :: stat
      logical, save :: cached = .false., known = .false.
      if (.not. known) then
         call get_environment_variable("DFTB_FFT_EWALD", val, status=stat)
         if (stat == 0) cached = trim(val) == "1" .or. trim(val) == "yes" .or. trim(val) == "true"
         known = .true.
      end if
      useFft = cached
      #:if WITH_MPI or WITH_SCALAPACK
         ! The accelerated solve has no distributed-memory reduction.
         useFft = .false.
      #:endif
   ...
   function hasAtomicMatrix(this) result(available)
      ...
      logical :: available
      available = allocated(this%invRMat) .and. this%stockMatrixReady_
   end function hasAtomicMatrix

   !> The library Ewald methods require a neutral cell. Charged 3d systems
   !> continue through DFTB+'s original interaction matrix.
   function canUseEwaldCode(this) result(canUse)
      class(TCoulomb), intent(in) :: this
      logical :: canUse
      real(dp) :: absoluteCharge
      canUse = useFftEwald() .and. this%boundaryCond_ == boundaryCondsEnum%pbc3d
   ...
   !> the stock engine.
   !>
   !> The point is that DFTB+ reports the total force on an atom and not its
   ...
   function useDirectEwald() result(useDirect)
      ...
      logical, save :: cached = .false., known = .false.
      if (.not. known) then
         call get_environment_variable("DFTB_EWALD_DIRECT", val, status=stat)
         if (stat == 0) then
            cached = trim(val) == "1" .or. trim(val) == "yes" .or. trim(val) == "true"
         else
            cached = .false.
         end if
         known = .true.
      ...
      useDirect = cached
   end function useDirectEwald

   !> The tolerance the direct engine requests.  A reference has to be pinned
   !> independently of the request being swept, otherwise it moves with the
   !> engine it is meant to judge, so DFTB_EWALD_DIRECT_TOL overrides the
   !> EwaldTolerance of the input for the direct engine alone.  Unset, the
   !> direct engine simply takes the input value like the others.
   function directEwaldTol(tolInput) result(tol)
      real(dp), intent(in) :: tolInput
      real(dp) :: tol
      character(len=32) :: val
      integer :: stat, ioStat
      real(dp) :: parsed
      real(dp), save :: cached = -1.0_dp
      logical, save :: known = .false.
      if (.not. known) then
         known = .true.
         call get_environment_variable("DFTB_EWALD_DIRECT_TOL", val, status=stat)
         if (stat == 0) then
            read (val, *, iostat=ioStat) parsed
            if (ioStat == 0 .and. parsed > 0.0_dp .and. parsed < 1.0_dp) cached = parsed
         end if
      end if
      tol = tolInput
      if (cached > 0.0_dp) tol = cached
   end function directEwaldTol
   ...
   !> it without building the interaction matrix; the direct path is a reference.
   !> DFTB+ stores coordinates and lattice vectors as columns, while the copied
   !> Ewald methods expect rows. Units pass through in atomic units.
   subroutine fftEwaldShift(this, coords, shift)
      class(TCoulomb), intent(in) :: this
      real(dp), intent(in) :: coords(:, :)
   ...
   end subroutine fftEwaldShift

   !> Add the FFT-Ewald energy gradient to `deriv`.  The engine returns the
   !> physical force F = -dU/dR; DFTB+'s deriv array is +dU/dR, so we subtract.
   subroutine fftEwaldAddGradients(this, coords, deriv)
      class(TCoulomb), intent(in) :: this
      real(dp), intent(in) :: coords(:, :)
      ...
      do iAt = 1, this%nAtom_
         deriv(:, iAt) = deriv(:, iAt) - force(:, iAt)
      end do
   end subroutine fftEwaldAddGradients

   !> Number of timed repetitions of the Ewald hot call, from DFTB_EWALD_NREP
   !> (default 1).  The thesis protocol is 15 repetitions after one warm-up with
   !> mean, sample standard deviation and minimum recorded; a single run of a
   !> hot call carries no spread and cannot meet it.  Values below one are
   !> clamped to one.
   function ewaldNrep() result(nRep)
      integer :: nRep
      character(len=16) :: val
      integer :: stat, ios
      ...
      logical, save :: known = .false.
      if (.not. known) then
         call get_environment_variable("DFTB_EWALD_NREP", val, status=stat)
         if (stat == 0) then
            read (val, *, iostat=ios) cached
            if (ios /= 0 .or. cached < 1) cached = 1
         end if
         known = .true.
      end if
      nRep = cached
   end function ewaldNrep

   !> Print a timed hot call as "<label> mean sd min nrep".  With one repetition
   !> the standard deviation is reported as zero and the line degenerates to the
   !> single measurement, so the parsers of the comparison scripts see the same
   !> format either way.
   subroutine printTiming(label, times)
      character(len=*), intent(in) :: label
      real(dp), intent(in) :: times(:)
      real(dp) :: mean, sd
      integer :: n
      n = size(times)
      mean = sum(times)/real(n, dp)
      sd = 0.0_dp
      if (n > 1) sd = sqrt(sum((times - mean)**2)/real(n - 1, dp))
      print *, label, mean, sd, minval(times), n
   end subroutine printTiming

   !> Print the per-atom potential (shift) when DFTB_PRINT_SHIFT is set, one line
   !> per atom tagged "ShiftPerAtom:", so the old-vs-new comparison can read the
   !> converged potential.  Off by default to avoid noise.
   subroutine maybePrintShift(shift, nAtom)
      ...
      logical, save :: cached = .false., known = .false.
      if (.not. known) then
         call get_environment_variable("DFTB_PRINT_SHIFT", val, status=stat)
         cached = (stat == 0 .and. (trim(val) == "1" .or. trim(val) == "yes" .or. trim(val) == "true"))
         known = .true.
      end if
      if (.not. cached) return
      do iAt = 1, nAtom
         print *, "ShiftPerAtom:", iAt, shift(iAt)
      end do
   end subroutine maybePrintShift
   ...
   subroutine updateCoords(this, env, neighList, coords, species)
      ...
      ! If process is outside of atom grid, skip invRMat calculation.  With the
      ! FFT-Ewald engine the pbc3d O(N^2) invRMat build is skipped entirely (the
      ! per-atom potential is rebuilt from coords+charges each SCC iteration in
      ! updateShifts); the cluster path is unchanged.
      this%stockMatrixReady_ = .false.
      if (allocated(this%invRMat)) then
         if (this%boundaryCond_ == boundaryCondsEnum%pbc3d) then
            if (.not. useFftEwald()) then
               block
                  integer(8) :: tic, toc, rate
                  integer :: iRep, nRep
                  real(dp), allocatable :: times(:)
                  ! One warm-up build, then nRep timed ones (DFTB_EWALD_NREP,
                  ! default 1).  The build is idempotent: repeating it only
                  ! rewrites the same matrix.
                  nRep = ewaldNrep()
                  allocate (times(nRep))
                  if (nRep > 1) then
                     call invRPeriodic(env, this%nAtom_, coords, this%neighList_, &
                         & this%gLatPoints_, this%alpha, this%volume_, this%invRMat)
                  end if
                  do iRep = 1, nRep
                     ...
                     times(iRep) = real(toc - tic, dp)/real(rate, dp)
                  end do
                  call printTiming("Ewald matrix build time [s]:", times)
                  this%stockMatrixReady_ = .true.
               end block
            end if
         else
   ...
   subroutine addGradients(this, env, coords, species, iNeighbour, img2CentCell, gradients, dQOut,&
      ...
      if (present(dQOutAtom)) then
         ...
         if (this%boundaryCond_ == boundaryCondsEnum%pbc3d) then
            if (canUseEwaldCode(this)) then
               ! FFT-Ewald engine: energy gradient = -(physical force) from one
               ! adjoint + three forward NFFTs (ik-differentiation).
               call fftEwaldAddGradients(this, coords, gradients)
            else
   ...
   subroutine updateShifts(this, env, orb, species, iNeighbour, img2CentCell)
      ...
      #:else
         if (canUseEwaldCode(this)) then
            ! FFT-Ewald engine: rebuild the per-atom potential (the shift) from
            ! coords + charges in O(N log N), replacing invRMat . deltaQAtom_.
            block
               integer(8) :: tic, toc, rate
               integer :: iRep, nRep
               real(dp), allocatable :: times(:)
               ! Same protocol as the stock matrix build above: one warm-up
               ! solve, then nRep timed ones.  The solve is idempotent.
               nRep = ewaldNrep()
               allocate (times(nRep))
               if (nRep > 1) call fftEwaldShift(this, this%coords_, this%shiftPerAtom_)
               do iRep = 1, nRep
                  call system_clock(tic, rate)
                  call fftEwaldShift(this, this%coords_, this%shiftPerAtom_)
                  call system_clock(toc)
                  times(iRep) = real(toc - tic, dp)/real(rate, dp)
               end do
               call printTiming("Ewald FFT solve time [s]:", times)
            end block
         else
            if (useFftEwald() .and. this%boundaryCond_ == boundaryCondsEnum%pbc3d &
                .and. .not. this%stockMatrixReady_) then
               call invRPeriodic(env, this%nAtom_, this%coords_, this%neighList_, &
                   & this%gLatPoints_, this%alpha, this%volume_, this%invRMat)
               this%stockMatrixReady_ = .true.
            end if
            block
               ...
               integer :: iRep, nRep
               real(dp), allocatable :: times(:)
               ! The stock engine's per-iteration cost is this matrix-vector
               ! product, not the one-time build timed in updateCoords.
               nRep = ewaldNrep()
               allocate (times(nRep))
               if (nRep > 1) call hemv(this%shiftPerAtom_, this%invRMat, this%deltaQAtom_, 'L')
               do iRep = 1, nRep
                  call system_clock(tic, rate)
               ...
               call printTiming("Ewald hemv time [s]:", times)
            end block
         end if
         print *, "Ewald energy:", sum(this%shiftPerAtom_*this%deltaQAtom_)*0.5_dp
         call maybePrintShift(this%shiftPerAtom_, this%nAtom_)
